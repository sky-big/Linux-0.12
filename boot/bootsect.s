...
! test指令的执行结果是(0x1000 & 0x0fff) = 0x0000,于是ZF标志置位.此时即下一条指令jne条件不成立.
	...
	mov ax,es
	test ax,#0x0fff
die:	jne die			! es must be at 64kB boundary	! es值必须位于64KB边界!
	xor bx,bx		! bx is starting address within segment	! bx为段内偏移.
rp_read:
! 接着判断是否已经读入全部数据.比较当前所读段是否就是系统数据末端所处的段(#ENDSEG),如果不是就跳转至下面ok1_read标号处继续读数据.否则退出子程序
! 返回.
	mov ax,es
...
! 能读入的字节数(64KB - 段内偏移位置),反算出此次需要读取的扇区数.
	...
	jnc ok2_read
	je ok2_read
! 若加上此次将读磁道上所有未读扇区时会超过64KB,则计算此时最多能读入的字节数: (64KB - 段内读偏移位置),再转换成需读取的扇区数.其中0减某数就是取该数64KB的补值.	
	xor ax,ax
	sub ax,bx
	shr ax,#9
ok2_read:
! 读当前磁道上指定开始扇区(cl)和需读扇区数(al)的数据到es:bx开始处.然后统计当前磁道上已经读取的扇区数并与磁道最大扇区数sectors作比较. 如果小于sectors说明当
! 前磁道上还有扇区未读.于是跳转到ok3_read处继续操作.
